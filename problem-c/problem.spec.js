...
describe('1. Page utilizes the Bootstrap CSS framework', () => {
  ...
  test('HTML links (only) the Bootstrap stylesheet', () => {
    ...
    expect(links.attr('href')).toMatch(/bootstrap\.(min\.)?css$/); //links to bootstrap
  });
...
describe('3. The header is styled as a Jumbotron component', () => {
  test("The header's a jumbotron", () => {
    expect($('header').hasClass('jumbotron')).toBe(true);
  })

  test('The jumbotron is fluid', () => {
    expect($('header').hasClass('jumbotron-fluid')).toBe(true);
  })

  test('The jumbotron has white text on a dark background', () => {
...
describe('6. Icon image position is responsive', () => {
  ...
  test('Card body columns are correct sizes', () => {
    ...
    cardBodies.each((i, el) => {
      let cols = $(el).children().children();
      expect(cols.eq(0).attr('class')).toEqual('col-sm-auto'); //first column is auto
