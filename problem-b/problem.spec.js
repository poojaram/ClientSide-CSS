...
describe('3. Has appropriate styling on screens 598px or larger', () => {
  ...
  test('for icon links', () => {
    ...
    expect(hamburgerIdRule.declarations[0].property).toEqual('display'); //has 'display' as property
    expect(hamburgerIdRule.declarations[0].value).toEqual('none'); //has display:none
    ...
    expect(socialIdRule.declarations[0].property).toEqual('display'); //has 'display' as property
    expect(socialIdRule.declarations[0].value).toEqual('block'); //has display:none
  })
})
...
describe('4. Has appropriate styling on screens 768px or larger', () => {
  ...
  test('for the background image', () => {
    ...
    let bodyProperties = bodyRules[0].declarations
                        ...
                        .map((r) => r.value)
                        .join(' ');
    expect(bodyProperties).toMatch("url('../img/splash-md.jpg')"); //background has url
    ...
    expect(htmlRule.declarations[0].property).toEqual('height'); //has 'height' as property
    expect(htmlRule.declarations[0].value).toEqual('100%'); //has height:100%
  })
  ...
  test('for the text shadow', () => {
    ...
    expect(mainRule.declarations[0].property).toEqual('text-shadow'); //has 'text-shadow' as property
    expect(mainRule.declarations[0].value.toLowerCase()).toEqual('1px 1px #153c43'); //has correct shadow
  })
})
...
describe('5. Has appropriate styling on screens 992px or larger', () => {
  ...
  test('for the background image', () => {
    ...
    expect(bodyRule.declarations[0].property).toEqual('background-image'); //has 'background-image' as property
    expect(bodyRule.declarations[0].value).toEqual("url('../img/splash-lg.jpg')"); //has correct image        
  })
  ...
  test('for the nav links', () => {
    ...
    expect(navLinkRule.declarations[0].property).toEqual('font-size'); //has 'font-size' as property
    expect(navLinkRule.declarations[0].value).toEqual('1.5rem'); //has correct size
  })
  ...
  test('for the text content', () => {
    ...
    expect(h1Rule.declarations[0].property).toEqual('font-size'); //has 'font-size' as property
    expect(h1Rule.declarations[0].value).toEqual('5rem'); //has correct size

    let h2Rule = _.find(lgRules, (r) => r.selectors.join().includes('h2'));
    expect(h2Rule.declarations[0].property).toEqual('font-size'); //has 'font-size' as property
    expect(h2Rule.declarations[0].value).toEqual('3rem'); //has correct suze
  })
})
...
describe('6. Has appropriate styling on screens 1200px or larger', () => {
  ...
  test('for the background image', () => {
    ...
    expect(bodyRule.declarations[0].property).toEqual('background-image'); //has 'background-image' as property
    expect(bodyRule.declarations[0].value).toEqual("url('../img/splash-xl.jpg')"); //has correct image        
  })
})